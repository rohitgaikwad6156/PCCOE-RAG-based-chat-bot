  ...
  return (
    <>
      <button
        ...
        disabled={isLoading}
        className="w-full py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-slate-500 text-slate-200 text-xs font-semibold shadow-sm flex items-center justify-center gap-2.5 transition-all group active:scale-[0.99]"
      >
        {isLoading ? (
      ...
      {/* Google Account Selector & Email Sign-In Modal */}
      {isModalOpen && (
          ...
          <div className="bg-slate-900 border border-slate-700/80 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative animate-scaleIn">
            ...
            <div className="pt-3 border-t border-slate-800">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1">
                ...
                Or enter any Google / Gmail ID:
              </div>
              <form onSubmit={handleCustomEmailSubmit} className="space-y-2">
                <input
                  type="email"
                  required
                  placeholder="name@gmail.com or @pccoe.edu.in"
                  value={emailInput}
